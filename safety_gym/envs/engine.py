...
class Engine(gym.Env, gym.utils.EzPickle):
        ...
        if self.task == "goal":
            ...
            if self.goal_button is not None:
                self.sim.model.geom_rgba[
                    self.model.geom_name2id(f"button{self.goal_button}")
                ] = COLOR_BUTTON
    ...
    def obs_vision(self):
        ...
        vision = self.sim.render(
            width, height, camera_id="vision", scene_option=self._vis_opt
        )
    ...
    def render(
        self, mode="human", camera_id=None, width=DEFAULT_WIDTH, height=DEFAULT_HEIGHT
    ):
